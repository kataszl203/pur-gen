...
def server(input, output, session):
    ...
    @render.ui
    def check_input():
        if input.uploaded_file() is not None:
        ...
        if substrates:
            ...
            if selected_size == 2:
                ...
                for rnum in range(len(reagents_smiles)):
                    reactions += '<br><b>Reaction %i:</b> %s + %s -> %s' % (
                        rnum + 1, reagents_smiles[rnum][0], reagents_smiles[rnum][1], products_smiles[rnum])
                ...
                for rnum in range(len(reagents_smiles)):
                    reactions += '<br><b>Reaction %i:</b> %s + %s + %s -> %s' % (
                        rnum + 1, reagents_smiles[rnum][0], reagents_smiles[rnum][1], reagents_smiles[rnum][2],
                        products_smiles[rnum])
                    products_info += '<br> <b> %i.</b> %s' % (rnum + 1, products_smiles[rnum])
                ...
                for rnum in range(len(reagents_smiles)):
                    reactions += '<br><b>Reaction %i:</b> %s + %s + %s + %s -> %s' % (
                        rnum + 1, reagents_smiles[rnum][0], reagents_smiles[rnum][1], reagents_smiles[rnum][2],
                        reagents_smiles[rnum][3], products_smiles[rnum])
                    products_info += '<br> <b> %i.</b> %s' % (rnum + 1, products_smiles[rnum])
